...
Description: Parametrizes brain surface for incomplete angiography data.
"""
...
class VoxelModel(object):
    ...
    def clone(self):
        ...
        return Voxel(data, dims, translate, self.scale, self.axis_order)
...
#         new_model = VoxelModel(np.array(chosen_points, dtype=int), model.dims, model.translate, model.scale,
#                                model.axis_order)
#         write_binvox(new_model, new_file)


# TODO: parametrize a slice
def get_slice_coordinate(voxel_data, dtype=np.int):
    return np.asarray(np.nonzero(voxel_data), dtype)
