...
class VascularNetwork():
    ...
    def add_branching_point(self, loc, pres=None):
        self.tree.add_node(self.node_count, loc=loc, pressure=pres, HS=None, relaxed=False)
        self.node_count += 1
    ...
    def add_vessel(self, node, neighbor_node, radius=None, flow=None):
        ...
        dis = np.linalg.norm(np.array(self.tree.nodes[node]['loc']) - np.array(self.tree.nodes[neighbor_node]['loc']))
    ...
    def reconnect(self):
        for leaf in self.leaves:
            min_dis = np.linalg.norm(np.array(self.tree.nodes[0]['loc']) - np.array(self.tree.nodes[leaf]['loc']))
            nearest_node = 0
            for node in self.tree.nodes:
                dis = np.linalg.norm(np.array(self.tree.nodes[node]['loc']) - np.array(self.tree.nodes[leaf]['loc']))
    ...
    def move_node(self, node, loc_new):
        ...
        for n in self.tree.neighbors(node):
            self.tree[node][n]['length'] = np.linalg.norm(np.array(self.tree.nodes[node]['loc']) - np.array(self.tree.nodes[n]['loc']))
    ...
    def update_strahler_order(self):
        ...
        while count_no_label != 0:
            ...
            count_no_label = np.count_nonzero(np.array([self.tree.nodes[n]['HS'] for n in self.tree.nodes]) == 0)
...
if __name__ == '__main__':
    ...
    nx.draw(VN.tree, locs)
    label1 = nx.get_node_attributes(VN.tree, 'HS')
